...
# === Icon image file paths (unused for robots now) ===
...
def agent_portrayal(agent):
    ...
    Robots now display their unique_id as colored text.
    """
    if isinstance(agent, RadioactivityAgent):
        return None
    # Robots: show their unique_id in their type color
    if isinstance(agent, GreenRobotAgent):
        return {"text": str(agent.unique_id), "color": "green", "zorder": 2}
    if isinstance(agent, YellowRobotAgent):
        return {"text": str(agent.unique_id), "color": "orange", "zorder": 2}
    if isinstance(agent, RedRobotAgent):
        return {"text": str(agent.unique_id), "color": "red", "zorder": 2}
    # Wastes and disposal still use icons
    if isinstance(agent, WasteAgent):
        if agent.waste_type == 'green':
            return {"shape": "image/green_waste.png", "size": 40, "zorder": 2}
        elif agent.waste_type == 'yellow':
            return {"shape": "image/yellow_waste.png", "size": 40, "zorder": 2}
        elif agent.waste_type == 'red':
            return {"shape": "image/red_waste.png", "size": 40, "zorder": 2}
...
def draw_agent(ax, portrayal, pos):
    ...
    Now supports text rendering for robots.
    """
    if portrayal is None:
        return
    x0 = pos[0] * CELL_SIZE + CELL_SIZE/2
    y0 = pos[1] * CELL_SIZE + CELL_SIZE/2
    # Text-based portrayal (robots)
    if "text" in portrayal:
        ax.text(x0, y0,
                portrayal["text"],
                ha='center', va='center',
                color=portrayal.get("color", "black"),
                fontsize=12,
                zorder=portrayal.get("zorder", 2))
        return
...
def CustomGrid(model):
    ...
    wall_positions = {agent.initial_pos for agent in model.custom_agents if isinstance(agent, wallAgent)}
    ...
    ax.imshow(background_image, extent=[0, model.width * CELL_SIZE, 0, model.height * CELL_SIZE], zorder=0, interpolation='nearest')
...
model_params = {
    ...
    "num_green_waste": {
        "type": "SliderInt", "value": 4, "min": 0, "max": 50, "step": 1,
